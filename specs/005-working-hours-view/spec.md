...
- **Configured** (working hours): Working hours are considered "configured" when a valid `{start, end}` object has been successfully saved to local storage at least once. An absent or unparseable storage value means working hours are _not_ configured.
