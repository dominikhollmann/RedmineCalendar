...
### User Story 3 — Warning on Outlook Drag-and-Drop Pre-fill (Priority: P3)

A team member drags an Outlook calendar event onto the calendar. The system pre-fills the issue field from the event metadata. If the resolved ticket is closed, the same warning badge appears — the user is never silently sent into a closed-ticket submission.

**Why this priority**: This path is less common than manual selection, and the Outlook drag-and-drop integration is already handled in a separate module. It is a valuable completeness item but can ship after the core stories.

**Independent Test**: Can be tested by dragging an Outlook event that maps to a closed ticket and verifying the warning appears in the pre-filled modal.

**Acceptance Scenarios**:

1. **Given** the user drags an Outlook event onto the calendar, **When** the system resolves the pre-filled ticket to one that is closed, **Then** the warning badge appears in the modal before the user interacts with it.
2. **Given** a pre-filled modal with a closed-ticket warning, **When** the user submits without changing the issue, **Then** submission is not blocked — the warning is informational only.
...
- **FR-006**: The closed-ticket check MUST also run when the issue field is pre-filled via Outlook drag-and-drop, applying the same non-blocking warning.
...
- **Closed-Ticket Warning Badge**: A transient UI element displayed beneath the issue field in the time-entry modal. Appears when `is_closed: true`; disappears when the issue changes or the field is cleared.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: The warning badge appears within 1 second of a closed ticket being selected or pre-filled, with no additional user action required.
- **SC-002**: 100% of closed-ticket selections and pre-fills show the warning; 0% of open-ticket selections show it (false-positive rate = 0).
...
- Drag-and-drop from Outlook follows the same non-blocking warning behaviour as manual ticket selection; the submission path is not blocked regardless of ticket status.
