...
- Q: Accessibility audit/remediation scope — three main surfaces only, or the full app? → A: **Audit the full app; this feature's remediation budget covers the three main surfaces (calendar, time-entry modal, settings) plus the mobile day-view rendering of those same surfaces.** Other surfaces (chatbot panel, in-app docs panel, voice-input UI, modals other than the time-entry modal) are audited and findings are recorded in `a11y-audit.md`, but their remediation may be deferred to a follow-up feature if it would substantially expand scope. Rationale: the user framed this as a "small changes" bundle, so the fix budget is bounded; but the audit itself is cheap and producing the full report now prevents a second round of audit work later.
- Q: Accessibility regression strategy — one-shot audit only, or a permanent CI gate? → A: **Permanent CI gate.** The Playwright UI test pipeline that already runs in CI gains an axe-core scan step covering the three remediated surfaces (calendar, open time-entry modal, settings) in both light and dark themes. A future regression that re-introduces a WCAG 2.2 Level A or AA failure on those surfaces fails CI. Rationale: a one-shot audit guarantees regression over months; running axe in the existing UI test pipeline costs ~1–2 seconds per surface and is incrementally cheap. Surfaces not remediated in this feature (chatbot, docs, voice) are scanned but their results are reported, not enforced, to avoid blocking unrelated work.
...
- **FR-013**: An accessibility audit MUST be performed against WCAG 2.2 Level AA covering every user-facing surface of the application: the calendar page (`index.html`) in both desktop and mobile day-view layouts, the time-entry modal (open state), the settings page (`settings.html`), the chatbot panel, the in-app docs panel, and the voice-input UI — in both light and dark themes.
- **FR-014**: The audit MUST be captured as a written artefact in the feature directory (`a11y-audit.md`), listing every finding, the WCAG criterion it violates, the severity (A vs AA), and a triage decision per finding: **fixed in this feature**, **deferred to a follow-up feature** (with rationale and an explicit follow-up issue link or owner), or **not applicable**. Surfaces outside the in-feature remediation scope (chatbot, docs, voice) MAY have their findings triaged as deferred, but the audit MUST still list them.
- **FR-015**: After remediation, an automated accessibility scan (axe-core via the Playwright UI pipeline, or equivalent) of the three remediated surfaces — calendar (desktop + mobile day-view), time-entry modal (open), settings — MUST report zero WCAG 2.2 Level A or Level AA violations in both light and dark themes. The scan MUST run as a Playwright UI test case so it is reproducible and re-runnable locally with `npm run test:ui`.
- **FR-015a**: The same axe-core scan MUST be wired into the project's existing CI pipeline as a permanent regression gate: a future change that re-introduces a Level A or AA violation on a remediated surface fails CI. Non-remediated surfaces MAY be scanned in CI for reporting purposes but MUST NOT block the build.
...
- **SC-005**: After accessibility remediation, an automated axe-core scan of the three remediated surfaces in both themes — calendar desktop view × {light, dark}, calendar mobile day-view × {light, dark}, time-entry modal (open) × {light, dark}, settings × {light, dark} — reports zero WCAG 2.2 Level A or Level AA violations: 8 zero-violation scans total. The same scan runs in CI on every PR and on `main` post-merge.
- **SC-006**: After accessibility remediation, a keyboard-only walkthrough of the three remediated surfaces (calendar → open time-entry modal → save → navigate to settings → toggle dark mode → return to calendar) completes without the user ever needing a pointing device, with every active control showing a visible focus indicator with at least 3:1 contrast against its adjacent background in both themes.
- **SC-006a**: The audit artefact (`a11y-audit.md`) covers every user-facing surface of the application (calendar desktop and mobile, time-entry modal, settings, chatbot panel, docs panel, voice-input UI), records every finding with its WCAG criterion and severity, and triages each finding as fixed / deferred (with owner) / not-applicable. Deferred findings on the three remediated surfaces are zero; deferred findings on other surfaces may be non-zero with documented rationale.
...
- The accessibility **audit** covers the entire application (every user-facing surface), but the **remediation budget for this feature** is bounded to the three main surfaces (calendar in desktop + mobile day-view layouts, time-entry modal, settings) — see Clarifications. Audit-only surfaces (chatbot, docs, voice) ship with their findings recorded and a triage decision, but their fix work may move to a follow-up feature.
- The axe-core accessibility scan is added to the Playwright UI test pipeline (one new test file or set of test cases), runs on every PR + post-merge `main` push as a permanent CI regression gate, and is locally re-runnable via `npm run test:ui`. A new dev dependency (`@axe-core/playwright` or the project's preferred equivalent) is introduced — this is a build-time-only dependency, not a runtime dependency for the SPA.
