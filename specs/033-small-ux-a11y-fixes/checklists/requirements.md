...
- **Clarifications session 2026-05-17** resolved three ambiguities: (1) ArbZG exemption applies to all six warning categories, not only break rules; (2) accessibility audit covers the full app, remediation budget covers three main surfaces + mobile day-view; (3) axe scan runs as a permanent CI regression gate inside the existing Playwright pipeline. See spec `## Clarifications` section for full rationale.
