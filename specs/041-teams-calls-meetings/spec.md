...
2. **Given** a Teams call (direct call, no issue inferable), **When** the user drags it to the Bookings column, **Then** the booking modal opens pre-filled with the rounded actual times and the participant list as a reference in the description field.
3. **Given** a Teams meeting whose title contains no issue reference, **When** the user drags it to the Bookings column, **Then** the booking modal opens with the rounded actual times and the meeting title visible; the user must provide an issue number before submitting.
...
1. **Given** a direct Teams call with participants Alice, Bob, and the signed-in user, **When** the Teams column renders the event, **Then** it displays "Alice, Bob" (or equivalent participant summary), not the user's own name.
...
- **Call with many participants**: Truncate the participant list in the event card with an overflow indicator (e.g. "Alice, Bob + 3 more"); the booking modal shows the full list.
...
- **Feasibility risk — API permissions**: Access to Teams call records may require tenant-admin consent for the `CallRecords.Read.All` permission. A feasibility spike (FR-015) MUST gate all implementation work. If per-user access without admin consent is confirmed unavailable, the Teams calls sub-feature may be limited to scheduled meetings only (which appear in the standard calendar Graph endpoint and require no additional permissions).
...
- **FR-005**: The Teams column MUST show all **direct (ad-hoc) calls** and **scheduled meetings** the user participated in for the selected Planning Day, using the **actual start and end times** recorded by Teams — not the originally scheduled times. Times MUST be displayed to the minute (no rounding in the display).
- **FR-006**: For **direct calls** (ad-hoc, no scheduled meeting behind them), the Teams column MUST display the names of all other participants (everyone except the signed-in user) as the event label. If no other participants exist, a localised fallback label MUST be shown.
...
- **FR-009**: Each Teams event MUST be classified and visually badged using the same three categories as Outlook events: **bookable** (issue identified), **needs-ticket** (work event, no issue found), or **excluded** (break, non-work, holiday — using the same exclusion rules as the Outlook column). Direct calls with no inferable issue are classified as needs-ticket.
- **FR-010**: Teams events MUST be draggable to the Bookings column using the same drag-and-drop interaction as the Outlook column. Multi-event shift-click selection and drag behaviour MUST be identical to the Outlook column (feature 038, FR-009b).
...
- **FR-012**: When a dragged Teams event has no identified issue, the booking modal MUST open pre-filled with the quarter-hour-rounded actual times. For calls, the participant list MUST be shown in the modal as context. For meetings, the meeting title MUST be shown. The user MUST provide an issue number before submitting.
- **FR-013**: A Teams event whose full time range is already covered by existing Redmine bookings MUST be visually greyed out in exactly the same manner as covered Outlook events (feature 038, FR-016).
...
- **FR-015**: A feasibility spike MUST be completed and documented before any Teams call-record integration is implemented. The spike MUST confirm: (a) whether per-user delegated access to Teams call history is available without tenant-admin consent, and (b) the minimum permission set required. **If per-user access to call records requires admin-consented permissions that are unavailable in the target deployment, the implementation MUST fall back to scheduled-meetings-only mode** (using the same Graph calendarView endpoint as the Outlook connector, filtered to Teams meetings). The fallback mode still satisfies FR-005 through FR-013 for the meetings sub-type; direct call display becomes out of scope pending admin enablement.
...
- A feasibility spike (FR-015) gates the call-records sub-feature. If admin-level permissions are required and unavailable, the Teams column ships in **meetings-only mode** using the standard calendarView Graph endpoint (same as Outlook), which requires no additional permissions beyond those already used.
- In meetings-only fallback mode, the Teams column shows scheduled meetings with actual end-time data where available, and falls back to scheduled end time if actual records are not accessible.
