...
- Q: How should the browser app submit feedback and interaction logs? → A: _(Revised)_ Via POST requests to a server-side feedback endpoint running alongside the shared CORS proxy. The endpoint writes JSON files to disk. This supports collecting feedback from all users in the shared deployment.
