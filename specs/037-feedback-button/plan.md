...
Add a persistently visible "Give Feedback" floating button (bottom-right corner) to both `index.html` and `settings.html`. Clicking it opens a modal dialog with a mandatory category selector ("Bug Report" / "Suggestion"). Bug Reports auto-collect a full diagnostic bundle: recent application errors + stack traces, a network-request log (via fetch proxy), non-sensitive localStorage values, and current FullCalendar state. An "Add Screenshot" button in the context section lets users capture a pixel-perfect screenshot on demand via `MediaDevices.getDisplayMedia()` (dialog closes temporarily, browser tab picker confirms, dialog re-opens with screenshot). Submission sends a rich HTML email with screenshot attachment via the existing Office 365 / MSAL integration when the user is signed in; falls back to a pre-filled `mailto:` link otherwise. The recipient address is admin-configured in `config.json` (`feedbackEmail`); the button is hidden entirely when that field is absent.
...
**Primary Dependencies**: FullCalendar v6 (CDN, existing); MSAL.js v2 (CDN, existing — extended for `Mail.Send`); `MediaDevices.getDisplayMedia()` (browser-native, no new dependency); no new npm or CDN runtime deps  
...
| II. Calendar-First UX   | ✅ Pass     | Floating button (bottom-right, fixed position) does not overlap calendar controls; slides left when the AI chat panel is open (CSS `--chatbot-panel-w` variable). Dialog is a native `<dialog>` modal — keyboard-dismissable. Screenshot capture is on-demand via `getDisplayMedia` — non-blocking; dialog closes during capture and re-opens with result.                                                                          |
...
| IV. Simplicity & YAGNI  | ✅ Pass     | No new CDN or npm dependency. `getDisplayMedia` is browser-native and produces pixel-perfect captures without CSS compatibility issues. No abstraction layers beyond the two new modules. `window.fetch` wrapping is minimal and idempotent.                                                                                                                                                                                        |
...
| VI. Quality Gates       | ✅ Required | CI must stay green: lint, typecheck, coverage ≥95% for new pure-logic module, SQI ≥ 80. No SBoM regeneration needed — no new CDN or npm dependency added.                                                                                                                                                                                                                                                                           |

**Complexity Tracking** (IV justification):

| Addition                      | Why Needed                                            | Simpler Alternative Rejected Because                                                                                                                                    |
| ----------------------------- | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `getDisplayMedia()` on-demand | Pixel-perfect screenshot; no CSS compatibility issues | `html2canvas` v1.4.1 fails on modern CSS (`color()` function); auto-capture before dialog opens cannot include dialog-state context; on-demand is more privacy-friendly |
