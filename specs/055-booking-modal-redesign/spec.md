...
5. **Given** any ticket row in any column, **When** its ticket number, subject, or project path is longer than the row width, **Then** the text truncates with an ellipsis on a single line (never wraps) and the full "#id subject — project" text is available as a hover tooltip using the app's unified tooltip style (`attachLabelTooltip` in `js/anomaly-render.js`).
...
- **FR-005**: Ticket rows MUST be single-line, fixed-height, truncated with an ellipsis (never wrapped), and MUST expose the full "#id subject — project" text as a hover tooltip using the app's unified tooltip style (not the browser-native `title` attribute), consistent with tooltips elsewhere in the app.
...
- **FR-010a**: The modal MUST be repositionable by dragging its header (added during UAT — the modal centers itself with a static position instead of live-recentering during resize, per FR-010's fix, which otherwise leaves no way to move it off-center). Position is session-only: it is NOT persisted, and the modal always re-centers on the next open.
