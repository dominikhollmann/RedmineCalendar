...
- [x] T001 [P] Create empty stub `js/anomalies.js` exporting `detectAnomalies`, `veryShortEntry`, `overlappingEntries` — placeholders that throw `'not implemented'`.
- [x] T002 [P] Create empty stub `js/anomaly-render.js` exporting `attachAnomalyBadge(eventEl, tag, t)`.
- [x] T003 [P] Create empty test files: `tests/unit/anomalies-rules.test.js`, `tests/unit/anomalies-aggregator.test.js`, `tests/ui/anomalies.spec.js`.
...
- [x] T004 [US1] Add the six EN+DE i18n keys from research.md §R6 to `js/i18n.js`. (Tagged US1 since it's the smaller-scope story; US2 reuses them.)
...
- [x] T005 [US1] In `tests/unit/anomalies-rules.test.js` write Vitest cases for `veryShortEntry(entry)`:
...
- [x] T006 [US1] Run `npx vitest run tests/unit/anomalies-rules.test.js` — Red.
- [x] T007 [US1] Implement `veryShortEntry(entry, t)` in `js/anomalies.js`. One-line predicate.
- [x] T008 [US1] Run vitest — Green.

### TDD: aggregator with break-ticket exclusion

- [x] T009 [US1] In `tests/unit/anomalies-aggregator.test.js` write Vitest cases for `detectAnomalies(entries, { breakTicket, holidayTicket })` covering ONLY the very-short rule (overlap rule cases come in US2):
...
- [x] T010 [US1] Run vitest — Red.
- [x] T011 [US1] Implement `detectAnomalies` in `js/anomalies.js` with the very-short rule wired in. Skip overlap rule for now (US2 will add it).
- [x] T012 [US1] Run vitest — Green.

### Implementation: rendering glue

- [x] T013 [US1] Implement `attachAnomalyBadge(eventEl, tag, t)` in `js/anomaly-render.js`: appends `<span class="fc-event__anomaly-badge" tabindex="0" role="button" aria-describedby="anomaly-tooltip-{id}">⚠</span>` and a sibling `<div class="anomaly-tooltip" role="tooltip" hidden>{reasons joined by line breaks}</div>`. Wire click handler to toggle `hidden`. Fire on focus too for keyboard users.
- [x] T014 [US1] In `js/calendar.js`'s `loadWeekEntries()` (after `enrichEntries(mapped)`, before `splitMidnightEntries(mapped)`), call `detectAnomalies(mapped, { breakTicket, holidayTicket })` and store on `window._calendarAnomalies` (mirroring the `_calendarDayTotals` pattern at `js/calendar.js:329`).
- [x] T015 [US1] Add an `eventDidMount(info)` callback to the FC config: looks up `info.event.id` (or `extendedProps.timeEntry.id`) in `window._calendarAnomalies`; if found, calls `attachAnomalyBadge(info.el, tag, t)`.

### Styling

- [x] T016 [US1] In `css/style.css` add `.fc-event__anomaly-badge` (small absolute-positioned badge in top-right of an FC event; ~12 px square; subtle warning colour; `pointer-events: auto`) and `.anomaly-tooltip` (small popover; `position: absolute`; shown on `:hover`/`:focus-within` of the badge or when its `hidden` attr is removed).

### UI tests

- [x] T017 [US1] In `tests/ui/anomalies.spec.js` add Playwright tests covering quickstart S1–S5 + S15 (tooltip dismisses with entry) + S16 (network invariant — assert zero new requests during anomaly evaluation; use `page.on('request')` to count).
- [x] T018 [US1] Run `npx playwright test tests/ui/anomalies.spec.js` — Red, then iterate T013–T016 until Green.
...
- [x] T019 [US2] Extend `tests/unit/anomalies-rules.test.js` with cases for `overlappingEntries(dayGroup)`:
...
- [x] T020 [US2] Run vitest — Red.
- [x] T021 [US2] Implement `overlappingEntries(dayGroup, t)` in `js/anomalies.js`. Pairwise scan. Return `Map<entryId, reasonString[]>`.
- [x] T022 [US2] Run vitest — Green.

### TDD: aggregator with overlap rule + multi-rule merge

- [x] T023 [US2] Extend `tests/unit/anomalies-aggregator.test.js` with cases:
...
- [x] T024 [US2] Run vitest — Red.
- [x] T025 [US2] Wire `overlappingEntries` into `detectAnomalies`: group by `spentOn` after filtering break entries; merge per-entry reasons with the very-short rule's results; produce the unified Map.
- [x] T026 [US2] Run vitest — Green.

### UI tests

- [x] T027 [US2] Extend `tests/ui/anomalies.spec.js` with Playwright cases for quickstart S6–S10, S11 (multi-rule), S12 (midnight split — rare; document if hard to reproduce in test), S13 (holiday not flagged), S14 (break never flagged), S17 (mobile).
- [x] T028 [US2] Run `npx playwright test tests/ui/anomalies.spec.js` — Red, then verify Green (no production-code changes likely needed since the rule engine already supports it; only re-render glue changes are if midnight-split needs special handling).
- [x] T029 [US2] If S12 (midnight-split) reveals that `eventDidMount` is called once per visual half (which it is), confirm both halves correctly look up `extendedProps.timeEntry.id` and both get badged. Add a unit test for the lookup if needed.
...
- [x] T030 [P] Run full Vitest suite (`npx vitest`) — no regressions.
- [x] T031 [P] Run full Playwright suite — no regressions, especially in entry CRUD specs (SC-004).
- [x] T032 [P] Manually walk every quickstart scenario S1–S17; check the dev-tools console for errors (must be zero).
- [x] T033 [P] Verify SC-002 timing: from `mouseup` on a saved entry change to badge appearance/disappearance < 300 ms.
- [x] T034 [P] Verify SC-003 network invariant by capturing a HAR for a CRUD-heavy session and asserting no requests beyond Redmine CRUD endpoints.
- [x] T035 Update BACKLOG.md row for 029: `plan ✅`, `tasks ✅`, status `tasks done — ready for implement`.
