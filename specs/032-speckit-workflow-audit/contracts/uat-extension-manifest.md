...
- `hooks.<step>.<n>.command` MUST refer to a command defined in the same extension's `commands` list, or to a command from another _installed_ extension (the resolver checks at hook execution time).
