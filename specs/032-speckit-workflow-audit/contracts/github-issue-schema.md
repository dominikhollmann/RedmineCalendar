...
| `specify` / `clarify` / `plan` / `tasks` / `implement` / `UAT` | Used to derive the _highest reached_ lifecycle stage; sets the `status:*` label accordingly. Done section rows always get `status:done` (closed) regardless of which columns are filled. |
