...
> **Note on "Phase 0 research"**: This refers to the up-front research work that produces `research.md` — done during `/speckit.plan` invocation, not as a separate scheduled phase in `tasks.md`. `tasks.md` numbers Phases 1+ (Setup → Spec Kit upgrade → US1/2/3 → Polish); the research artifact is an input to those phases, not a task in them.
...
| **VI. Continuous Quality Gates** | ✅ Pass — added retroactively                        | Principle VI was added to the constitution on 2026-05-12 (after this plan was written). Verified satisfied: T054 runs the full local pipeline (`npm run lint && format:check && typecheck && test && sqi`); `after_implement` hooks include `speckit.git.test` as the post-implement gate; PR CI runs the same checks before merge; release.yml gates version bumps behind CI green. No SQI band regression introduced.                                                                              |
