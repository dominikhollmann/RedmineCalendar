...
4. **Given** a user asks a question in German, **When** the chatbot processes it, **Then** the response is also in German (matching the user's language).
5. **Given** a user asks something completely outside the scope of the application (e.g. "What is the weather today?"), **When** the chatbot responds, **Then** it politely declines and redirects the user to application-related topics.
