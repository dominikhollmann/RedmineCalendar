...
| Markdown rendering — `chatbot.js` vs `docs.js` (planned `markdown.js`, clones #12/#13) | **Not unified** (reverses the plan's "will fix P2"; documented here per Scenario 6) | The two render **different trust domains**: `docs.js renderMarkdown` parses bundled, admin-authored `docs/content.*.md` (trusted) with a lightweight hand-rolled parser (headings/tables/lists, no external dep); `chatbot.js renderMessage` parses **untrusted LLM output** via `marked` + a mandatory `DOMPurify.sanitize` boundary (FR-009, hardened in 035). Unifying would either pull `marked`+DOMPurify into trusted-content rendering or dissolve the sanitisation boundary that exists precisely because the chatbot input is untrusted — a Constitution IV/V regression. Under jscpd 5.x the two are no longer even token-clones (0 detected). Sanitisation is preserved on both sides; nothing was converged by guessing. |
