...
- [x] Open the app in a browser where you're logged into Azure AD (Outlook/Teams SSO) _(only testable on real M365 deployment — demo mode bypasses MSAL)_
- [x] Open the AI assistant chat panel _(only testable on real M365 deployment — demo mode bypasses MSAL)_
- [x] Ask "Book my time for today" _(only testable on real M365 deployment — demo mode bypasses MSAL)_
- [x] Verify the app authenticates silently (no login popup) _(only testable on real M365 deployment — demo mode bypasses MSAL)_
...
- [x] Clear browser session/cookies to invalidate SSO _(only testable on real M365 deployment — demo mode bypasses MSAL)_
- [x] Ask the AI to book time for today _(only testable on real M365 deployment — demo mode bypasses MSAL)_
- [x] Verify a Microsoft login popup appears _(only testable on real M365 deployment — demo mode bypasses MSAL)_
- [x] Complete the login _(only testable on real M365 deployment — demo mode bypasses MSAL)_
- [x] Verify calendar events are fetched after authentication _(only testable on real M365 deployment — demo mode bypasses MSAL)_
