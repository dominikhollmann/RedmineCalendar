...
Nach der Migration gilt: `grep -rE '#[0-9a-fA-F]{3,8}|rgb\(|rgba\(|hsl\(' css/` darf ausschließlich Treffer **innerhalb von `var(--*)` Definitionen** in `base.css` `:root`-Blöcken liefern — also in den Zeilen, die eine Variable _definieren_, nicht in Property-Wert-Zuweisungen.
