...
| `-4px 0 16px …` (docs side panel)           | docs.css:36,206                            | **Exception** (as-built) — directional drawer shadow casting left from the right-edge panel; off the vertical `--shadow-*` ramp by design; escape-hatch with rationale |
...
- **INV-3**: The `:root` token-definition blocks need **no** gate exemption — they define custom properties (`--font-*`, `--radius-*`, `--shadow-*`, `--space-*`), not the gated `font-size`/`border-radius`/`box-shadow`/spacing properties, so the rule never inspects them. (No block-level disable is used anywhere; only per-line `disable-next-line` for genuine exceptions.)
