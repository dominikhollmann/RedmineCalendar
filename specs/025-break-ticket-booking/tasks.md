...
- [x] T001 Add CSS rules in `css/style.css`: `.fc-event--break` (muted gray background + small "0h" badge per FR-010 / research R3). The `.input--locked` rule was added during 025 and removed by feature 026 (no consumers post-modal-redesign). Visual coverage is provided by the unit test `tests/unit/calendar-render.test.js` (asserts the `fc-event--break` class is applied) plus the Playwright spec `tests/ui/modal-hours-lock.spec.js` (asserts `.info-dur--break` styling); the dedicated `tests/ui/break-styling.spec.js` originally cited here was not created — coverage was consolidated.
...
- [x] T012 [P] [US1] Vitest unit-test coverage for the AI break-routing flow. **Coverage was consolidated into the existing `tests/unit/chatbot-tools.test.js`** (the dedicated `chatbot-break.test.js` originally cited was never created): see the `executeTool — book_outlook_day` describe block (groups break-routed events under the auto-routed section, separates needs-input proposals, etc.) plus the `outlook.test.js` parser tests covering `category: 'break'` proposals and the per-FR `i18n.test.js` keys. The modal hours-lock invariant (FR-012, redesigned to `0m (break)` per FR-019) is covered by `tests/ui/modal-hours-lock.spec.js`.
...
- [x] T016 [P] [US1] Add Playwright UI test in `tests/ui/booking-flow-break.spec.js` (NEW): with mocked Outlook events (one "Doctor Appointment 14:00–15:00", one "Sprint Planning #2097 09:00–10:00") and a mocked AI that classifies "Doctor Appointment" as non-work, run the booking flow. Assert (a) proposal summary shows both events with ticket number AND title, (b) confirming the doctor entry creates a Redmine POST with `hours: 0` and `issueId: breakTicket`, (c) the calendar after refresh shows the 0h entry with `.fc-event--break` class and the synthetic 15-min display.
- [x] T017 [US1] **Resolution: no code change needed.** Per spec.md Q1 clarification ("Same flow as any other ticket — confirmation goes through the existing time-entry modal"), the modal MUST open for break entries too. The hours-lock invariant (FR-012) handles 0h enforcement: when the AI calls `create_time_entry` with `issueId === breakTicket`, `executeCreate` opens the modal via `openForm`, `_selectedIssue` is set from the prefill, `updateTicketInfo` runs, and `applyHoursLock` locks the End input automatically. Verified by reading the call chain `js/chatbot-tools.js:executeCreate → openForm → updateTicketInfo → applyHoursLock`. The Q1 clarification overrode the "skip modal" research direction documented in `contracts/classifier.md`.
- [x] T018 [US1] Manual UAT: run `quickstart.md` UAT-3 (AI classifies a non-work event) and UAT-4 (sensitivity ignored). Both must pass.
...
- [x] T024 [P] [US3] Add Playwright UI test in `tests/ui/settings-page-no-tickets.spec.js` (NEW): load `/settings.html`, assert the page contains an input for "Weekly hours" but NO element matching `[id="holidayTicket"]` or `[id="breakTicket"]`, no label whose text contains "Holiday ticket" or "Break ticket" (in either EN or DE per i18n).
...
- [x] T027 Run the full `quickstart.md` UAT script against the dev server (UAT-1 through UAT-10). Record pass/fail in the document and resolve any failures before declaring the feature done.
- [x] T028 [P] Visual review: open `index.html` and exercise the booking flow end-to-end. Confirm `.fc-event--break` is visually distinct from work entries (color, badge), the synthetic 15-min display block is clickable, the modal's `.input--locked` treatment is unambiguous, and the "Break (0h)" label is legible at typical zoom levels. Note any tweaks in a follow-up issue rather than blocking this feature.
