...
- Run classification _inside_ `parseCalendarProposals` — rejected: makes the parser non-deterministic and forces all unit tests to mock the LLM.
...
**Decision**: When mapping a 0-hour time entry to a FullCalendar event in `js/calendar.js`, give the FC event a _display_ end of `start + 15min` and tag it with a CSS class `fc-event--break`. The underlying `entry.hours = 0` is preserved in `extendedProps.timeEntry` for totals; only the visual span is stretched. The `.fc-event--break` class gets a distinct color (e.g. muted gray) and a small "0h" badge so the user does not mistake it for a 15-minute work entry.
