...
### User Story 3 - Navigate to a Different Day (Priority: P2)

A user wants to review and fill gaps from yesterday or plan bookings for tomorrow by switching the Planning View to a different date.

**Why this priority**: While the current day is the default, users frequently need to log time for previous days. Day navigation is a prerequisite for the Planning View to be useful beyond the immediate "now".

**Independent Test**: Can be fully tested by navigating to yesterday in the Planning View and verifying that both columns update to show that date's data.

**Acceptance Scenarios**:

1. **Given** the Planning View is open on today, **When** the user navigates to yesterday, **Then** both the Bookings column and the Outlook column refresh to show that date's data.
2. **Given** the user navigates to a future date, **When** the Outlook column loads, **Then** future appointments for that day are shown (if any exist).
3. **Given** the user navigates away from today, **When** they use a "Today" shortcut, **Then** the view returns to the current date.

---

### User Story 4 - Configure Event Source Columns (Priority: P2)
...
### User Story 5 - Time-Covered Events Are Greyed Out (Priority: P2)
...
- **FR-001**: The application MUST provide a dedicated Planning View accessible from the main navigation.
- **FR-002**: The Planning View MUST display one day at a time, defaulting to the current date, with controls to navigate to the previous and next day and a shortcut to return to today.
- **FR-003**: The Planning View MUST display a "Bookings" column showing existing Redmine time entries for the selected day, using the same data source as the main calendar view.
- **FR-004**: When Outlook is connected and enabled, the Planning View MUST display an "Outlook" column showing the user's Outlook calendar appointments for the selected day.
- **FR-005**: When Outlook is not connected or the user has disabled it in settings, the Planning View MUST display an appropriate prompt or empty state in place of the Outlook column.
- **FR-006**: The user MUST be able to initiate a Redmine time entry booking from any Outlook event displayed in the Planning View by double-clicking the event, consistent with the existing calendar's double-click-to-edit interaction pattern.
- **FR-007**: When a booking is initiated from an Outlook event, the time entry form MUST be pre-filled with the event's start time and end time.
- **FR-008**: The application MUST attempt to infer the Redmine issue number from the Outlook event's title and description using the same pattern-matching and AI logic already used in the chatbot feature; if inference succeeds, the issue field in the booking form MUST be pre-populated (user may override).
- **FR-009**: If no issue number can be inferred, the booking form MUST open with the time pre-filled but the issue field empty, requiring the user to provide it before submitting.
- **FR-010**: Users MUST be able to enable or disable individual event source columns from the Settings page; the setting MUST persist across sessions.
- **FR-011**: The Planning View MUST handle an expired or revoked Outlook authentication by displaying a clear, actionable reconnect prompt rather than an error state or blank column.
- **FR-012**: Tracking which specific Outlook events have been manually converted into bookings is out of scope for v1 and will be addressed in a follow-up feature.
- **FR-013**: Any Outlook event whose entire time range is fully covered by one or more existing Redmine bookings in the Bookings column MUST be visually distinguished (e.g., greyed out) to indicate the time has already been accounted for. This state is computed at render time from the existing bookings data — no additional storage is required.
...
- The Planning View is a separate page or route from the main calendar view, not an overlay or modal on top of it.
