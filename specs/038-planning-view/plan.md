...
| `contentHeight: 'auto'` on Bookings FullCalendar   | Disables FullCalendar's internal scroll so both columns share a single outer scroll container; required for pixel-accurate time alignment between Bookings slots and Outlook cards | JavaScript scroll-event sync across FullCalendar's nested internal scroll containers is fragile and causes jitter                                                 |
