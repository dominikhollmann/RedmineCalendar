...
- JavaScript ES2022 (vanilla ES modules, no transpilation, no build step) + FullCalendar v6 (CDN, existing); existing `js/redmine-api.js` (`updateTimeEntry`, `deleteTimeEntry`); no new deps (claude/plan-tasks-028-bulk-select-move-delete)
- in-memory `Set<entryId>` only — no localStorage, no IndexedDB (claude/plan-tasks-028-bulk-select-move-delete)
...
## Recent Changes
- claude/plan-tasks-028-bulk-select-move-delete: Added JavaScript ES2022 (vanilla ES modules, no transpilation, no build step) + FullCalendar v6 (CDN, existing); existing `js/redmine-api.js` (`updateTimeEntry`, `deleteTimeEntry`); no new deps
