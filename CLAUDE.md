...
- 036-css-refactor: Added `stylelint` + `stylelint-config-standard` (both dev-only). Replaced monolithic `css/style.css` (2122 lines) with 5 component files (`css/base.css`, `css/calendar.css`, `css/time-entry.css`, `css/docs.css`, `css/settings.css`). All hardcoded hex/rgb color values in CSS rules replaced with `var(--*)` references — 19 new design tokens added. Stylelint gate (`color-no-hex`, `color-named`, `function-disallowed-list`) + HTMLHint `style-disabled` rule now enforced in `npm run lint`. `sbom.json` + `attributions.json` regenerated. Plan: `specs/036-css-refactor/plan.md`.
