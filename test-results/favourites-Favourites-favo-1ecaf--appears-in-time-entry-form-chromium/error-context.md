...
- Name: favourites.spec.js >> Favourites >> favourites section appears in time entry form
- Location: tests/ui/favourites.spec.js:13:3
...
Locator: locator('text=/Favourites|Favoriten/i').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=/Favourites|Favoriten/i').first()
...
```ts
  ...
  11 |   });
  12 | 
  13 |   test('favourites section appears in time entry form', async ({ page }) => {
  14 |     const slot = page.locator('.fc-timegrid-slot[data-time="10:00:00"]').first();
  15 |     await slot.click();
  16 |     const favouritesHeading = page.locator('text=/Favourites|Favoriten/i');
> 17 |     await expect(favouritesHeading.first()).toBeVisible({ timeout: 5000 });
     |                                             ^ Error: expect(locator).toBeVisible() failed
  18 |   });
  19 | });
  20 | 
