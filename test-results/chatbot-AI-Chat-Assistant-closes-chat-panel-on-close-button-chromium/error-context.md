...
- Name: chatbot.spec.js >> AI Chat Assistant >> closes chat panel on close button
- Location: tests/ui/chatbot.spec.js:26:3
...
Call log:
  ...
  - waiting for locator('#chatbot-panel')
    3 × locator resolved to <div role="dialog" id="chatbot-panel" aria-modal="false" class="chatbot-panel" aria-label="AI Assistant">…</div>
      - unexpected value "visible"
    4 × locator resolved to <div hidden="" role="dialog" id="chatbot-panel" aria-modal="false" class="chatbot-panel" aria-label="AI Assistant">…</div>
      - unexpected value "visible"
...
- generic [ref=e1]:
  ...
  - dialog "AI Assistant" [ref=e502]:
    - generic [ref=e504]:
      - heading "AI Assistant" [level=2] [ref=e505]
      - button "Close" [active] [ref=e506] [cursor=pointer]: ✕
    - paragraph [ref=e509]: Hi! I can help you with RedmineCalendar. Ask me anything about the app.
    - generic [ref=e510]:
      - textbox "Ask about RedmineCalendar…" [ref=e511]
...
```ts
  ...
  2  | import { setupConfig, mockRedmineApi, mockAiApi, setupCredentials } from './helpers.js';
  3  | 
  4  | test.describe('AI Chat Assistant', () => {
  5  |   test.beforeEach(async ({ page }) => {
  6  |     await setupCredentials(page);
  7  |     await setupConfig(page);
  8  |     await mockRedmineApi(page);
  9  |     await mockAiApi(page);
  10 |     await page.goto('/index.html');
  11 |     await page.waitForSelector('.fc-event', { timeout: 10000 });
  12 |   });
  13 | 
  14 |   test('opens chat panel on button click', async ({ page }) => {
  15 |     await page.click('.chatbot-open-btn');
  16 |     const panel = page.locator('#chatbot-panel');
  17 |     await expect(panel).toBeVisible();
  18 |   });
  19 | 
  20 |   test('shows welcome message', async ({ page }) => {
  21 |     await page.click('.chatbot-open-btn');
  22 |     const messages = page.locator('.chatbot-msg');
  23 |     await expect(messages.first()).toBeVisible();
  24 |   });
  25 | 
  26 |   test('closes chat panel on close button', async ({ page }) => {
  27 |     await page.click('.chatbot-open-btn');
  28 |     await page.click('.chatbot-panel__close');
  29 |     const panel = page.locator('#chatbot-panel');
> 30 |     await expect(panel).toBeHidden({ timeout: 3000 });
     |                         ^ Error: expect(locator).toBeHidden() failed
  31 |   });
  32 | });
  33 | 
