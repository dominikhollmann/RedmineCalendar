...
```json
{
  ...
  "privacyControllerName": "[Controller name — set before go-live]",
  "privacyControllerEmail": "[Controller email — set before go-live]",
  "privacyDpoEmail": "[DPO email — set before go-live]",
...
| `feedback`                          | No                       | Object configuring the in-app **Give Feedback** button (feature 049). When absent, the button is hidden for all users. `feedback.system` selects the target: `"redmine"` files a ticket via the user's stored API key; `"github"` opens a prefilled GitHub new-issue form (no token stored). See the sub-fields below.                                      |
| `feedback.system`                   | If `feedback` set        | `"redmine"` or `"github"` — which tracker receives feedback tickets.                                                                                                                                                                                                                                                                                        |
| `feedback.redmineProjectId`         | Redmine only             | Numeric Redmine project ID the feedback issue is created in.                                                                                                                                                                                                                                                                                                |
| `feedback.redmineTrackerBug`        | No                       | Numeric Redmine `tracker_id` for **Bug Report** feedback (e.g. the "Problem" tracker). When unset (`0` or absent), the project's default tracker is used.                                                                                                                                                                                                   |
| `feedback.redmineTrackerSuggestion` | No                       | Numeric Redmine `tracker_id` for **Suggestion** feedback (e.g. the "Task"/"Feature" tracker). When unset (`0` or absent), the project's default tracker is used.                                                                                                                                                                                            |
...
| `bookingDeadline`                   | No                       | Object enabling a soft weekly **booking-deadline** warning. When `bookingDeadline.enabled` is `true`, creating, moving, or deleting a time entry for a week whose deadline has already passed shows a "continue anyway?" confirmation. When absent or `enabled: false`, no deadline warnings appear. See the sub-fields below.                              |
| `bookingDeadline.enabled`           | If `bookingDeadline` set | Boolean. Master switch for the deadline warning. Defaults to `false`.                                                                                                                                                                                                                                                                                       |
| `bookingDeadline.dayOfWeek`         | No                       | Day the weekly deadline falls on, as a JavaScript weekday index (`0` = Sunday … `6` = Saturday). Defaults to `5` (Friday).                                                                                                                                                                                                                                  |
| `bookingDeadline.hour`              | No                       | Hour of the deadline in 24-hour local time (`0`–`23`). Defaults to `22`.                                                                                                                                                                                                                                                                                    |
| `bookingDeadline.minute`            | No                       | Minute of the deadline (`0`–`59`). Defaults to `0`.                                                                                                                                                                                                                                                                                                         |
| `privacyControllerName`             | Before go-live           | Legal name of the data controller, shown in the in-app privacy notice (`privacy.html`, GDPR Art. 13(1)(a)). Used by feature 044.                                                                                                                                                                                                                            |
| `privacyControllerEmail`            | Before go-live           | Contact email of the data controller, shown in the privacy notice (Art. 13(1)(a)).                                                                                                                                                                                                                                                                          |
| `privacyDpoEmail`                   | No                       | Data Protection Officer contact email (Art. 13(1)(b)). Omit if your organization has no mandatory DPO — the field is hidden from the notice.                                                                                                                                                                                                                |
| `planningDataRetentionDays`         | No                       | Number of days planning-view snapshots (AI consent record + planning data) are retained in the user's browser before automatic purge. Defaults to `30`. Used by feature 044.                                                                                                                                                                                |
