...
GitHub Issues replace the old `BACKLOG.md` ledger. The lifecycle labels (`status:specify` … `status:done`) are the single source of truth for "where is this feature." See `.specify/extensions/github-issues/` for the Issues hook implementation, `.specify/extensions/publish/` for the publish hook, `.specify/extensions/uat/` for the UAT skill, and `specs/032-speckit-workflow-audit/contracts/github-issue-schema.md` for the Issue schema.
