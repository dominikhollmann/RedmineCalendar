...
4. **Build the body.** Per `contracts/github-issue-schema.md` § "Issue body format":
   ...
   Body links MUST use the project's current feature-directory path (`.specify/features/NNN-name/` pre-Phase-5d, `specs/NNN-name/` post-Phase-5d). The migration script (`scripts/migrate-backlog-to-issues.mjs`) is idempotent and patches stale body links on re-run.
