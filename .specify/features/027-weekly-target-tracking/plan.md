...
1. **Holiday-ticket admin field availability**: The spec assumes "Holiday/OOO entries (booked via existing holiday-ticket plumbing) treat the booked day as filled." Today `js/config.js` exports `STORAGE_KEY_HOLIDAY_TICKET` but no source file appears to read it. The admin-managed `cfg.holidayTicket` from `config.json` is the documented source per CLAUDE.md. Implementation should read it the same way `cfg.breakTicket` is read at `js/calendar.js:53`. If `cfg.holidayTicket` is absent, the feature falls back to "any non-zero booked entry counts as filled" — no spec violation, just less precise.
