...
**Input**: design docs in `/home/dominik/RedmineCalendar/.specify/features/032-speckit-workflow-audit/`
...
- [ ] T004 Run `specify integration upgrade claude --dry-run` from the repo root and capture the file-list output to `/home/dominik/RedmineCalendar/.specify/features/032-speckit-workflow-audit/upgrade-filelist.txt`. This is the canonical list of Spec-Kit-managed files that need 3-way merge per FR-015.
- [ ] T005 Update `/home/dominik/RedmineCalendar/.specify/init-options.json`: set `"speckit_version": "0.8.7"`. Single-line JSON edit.
...
- [ ] T006 [P] 3-way merge `/home/dominik/RedmineCalendar/.specify/templates/spec-template.md` (base = 0.6.1 vanilla, ours = current, theirs = 0.8.7 vanilla). Resolve conflicts; record decision per file in `/home/dominik/RedmineCalendar/.specify/features/032-speckit-workflow-audit/upgrade-decisions.md`.
- [ ] T007 [P] 3-way merge `/home/dominik/RedmineCalendar/.specify/templates/plan-template.md`. Same procedure as T006.
- [ ] T008 [P] 3-way merge `/home/dominik/RedmineCalendar/.specify/templates/tasks-template.md`. Same procedure.
- [ ] T009 [P] 3-way merge `/home/dominik/RedmineCalendar/.specify/templates/checklist-template.md`. Same procedure.
- [ ] T010 [P] 3-way merge `/home/dominik/RedmineCalendar/.specify/templates/agent-file-template.md`. Same procedure.
- [ ] T011 [P] 3-way merge `/home/dominik/RedmineCalendar/.specify/templates/constitution-template.md`. Same procedure (note: our constitution at `.specify/memory/constitution.md` is project content, not the template — DO NOT touch it here).
...
- [ ] T015 Write a paragraph in `/home/dominik/RedmineCalendar/.specify/features/032-speckit-workflow-audit/upgrade-decisions.md` summarising the per-file resolutions: total files merged, count by resolution (`accept-upstream` / `re-apply-ours` / `extension-hook`), and any noteworthy upstream changes (e.g. removed deprecated fields, new template sections).
...
- [ ] T017 [US1] Create directory `/home/dominik/RedmineCalendar/.specify/extensions/github-issues/`.
- [ ] T018 [US1] Write `/home/dominik/RedmineCalendar/.specify/extensions/github-issues/extension.yml` with the manifest defined in `contracts/uat-extension-manifest.md` § "Sibling extension: github-issues" (name, version, requires.speckit_version ≥ 0.8.7, gh ≥ 2.0.0; commands list of `github-issues.create` + `github-issues.update-status`; hooks bound to after_specify, after_clarify, after_plan, after_tasks, after_implement).
- [ ] T019 [P] [US1] Create `/home/dominik/RedmineCalendar/.specify/extensions/github-issues/commands/create.md`. The slash command body: read `FEATURE_DIR/spec.md`, extract feature number + title, build the templated body per `contracts/github-issue-schema.md`, call `gh issue create --title "Feature NNN: …" --body … --label feature --label status:specify`. MUST be idempotent: skip if `gh issue list --search "in:title \"Feature NNN:\"" --state all` returns a hit.
- [ ] T020 [P] [US1] Create `/home/dominik/RedmineCalendar/.specify/extensions/github-issues/commands/update-status.md`. The slash command body: accept `--status <name>` arg; look up Issue by `Feature NNN:` title regex; remove existing `status:*` label; add `status:<name>`. No-op if no Issue exists. Reads `FEATURE_DIR` from environment as set by the Spec Kit hook executor.
- [ ] T021 [P] [US1] Create `/home/dominik/RedmineCalendar/.specify/extensions/github-issues/workflows/issue-lifecycle.yml` per `contracts/github-issue-schema.md` § "Lifecycle workflow contract": triggers on `pull_request: closed (merged)`, parses `Closes #N` from PR body, sets `status:done` + `version:vX.Y.Z` (from latest matching `git tag`), closes the Issue.
- [ ] T022 [P] [US1] Write `/home/dominik/RedmineCalendar/.specify/extensions/github-issues/README.md` documenting the extension per the layout in `contracts/uat-extension-manifest.md` § README.md shape. Sections: requirements, usage, behavior, opt-out.

### Phase 3b: Wire up + install

- [ ] T023 [US1] Copy `/home/dominik/RedmineCalendar/.specify/extensions/github-issues/workflows/issue-lifecycle.yml` to `/home/dominik/RedmineCalendar/.github/workflows/issue-lifecycle.yml` (this is the only file the extension installs outside its own directory; per `contracts/uat-extension-manifest.md`).
- [ ] T024 [US1] Update `/home/dominik/RedmineCalendar/.specify/extensions.yml`: add `github-issues.create` to `after_specify` hooks; add `github-issues.update-status` (with `args.status: <phase>`) to `after_clarify`, `after_plan`, `after_tasks`, `after_implement` hooks. Replace the existing `git.commit` entries on those keys (per the per-hook proposal in `contracts/spec-kit-extensions-yml.md`). Run `specify extension install .specify/extensions/github-issues` to register the extension if `specify` requires it (verify by checking `specify extension list`).

### Phase 3c: Migration script

- [ ] T025 [US1] Create `/home/dominik/RedmineCalendar/scripts/migrate-backlog-to-issues.mjs`. Node ESM script. Parses `BACKLOG.md`, iterates each table row (skipping header + separator), maps the row to an Issue per `contracts/github-issue-schema.md` § "Migration: mapping BACKLOG.md rows to Issues" (Done rows → closed with `version:vX.Y.Z`; in-flight rows → open with `status:<highest reached>`). Idempotent: checks for existing `Feature NNN:` title before creating. Supports `--dry-run` flag (prints what would be done, makes no API calls).
- [ ] T026 [US1] Run the migration script in `--dry-run` mode against the current `BACKLOG.md` and verify the planned action for each row (~31 rows). Save the dry-run output as `/home/dominik/RedmineCalendar/.specify/features/032-speckit-workflow-audit/migration-dryrun.txt` for review before live execution.
...
- [ ] T028 [US1] Delete `/home/dominik/RedmineCalendar/BACKLOG.md` (per FR-005).
- [ ] T029 [US1] Update `/home/dominik/RedmineCalendar/.claude/commands/speckit.specify.md`: remove the BACKLOG.md row-append step (currently §8 "Update BACKLOG.md"). Replace with a short note: "Issue creation now happens via the `after_specify` hook bound to the `github-issues.create` extension command. See `.specify/extensions/github-issues/`."

### Phase 3e: Documentation

- [ ] T030 [P] [US1] Update `/home/dominik/RedmineCalendar/CONTRIBUTING.md`: replace all `BACKLOG.md` references with "GitHub Issues"; update the Spec Kit workflow section to mention the new lifecycle labels + the `issue-lifecycle.yml` close-on-merge behavior.
- [ ] T031 [P] [US1] Update `/home/dominik/RedmineCalendar/CLAUDE.md`: same scope as T030 — drop BACKLOG.md references, document the Issues-driven flow.
...
- [ ] T032 [US2] Create directory `/home/dominik/RedmineCalendar/.specify/extensions/uat/`.
- [ ] T033 [US2] Write `/home/dominik/RedmineCalendar/.specify/extensions/uat/extension.yml` per `contracts/uat-extension-manifest.md` § extension.yml schema. Single command (`uat.run`), no automatic hooks (human-invoked only).
- [ ] T034 [US2] Copy the body of `/home/dominik/RedmineCalendar/.claude/commands/speckit.uat.md` to `/home/dominik/RedmineCalendar/.specify/extensions/uat/commands/run.md`. Apply the modifications listed in `contracts/uat-extension-manifest.md` § "commands/run.md shape": (a) drop the local-merge step entirely; (b) drop the `git push origin --delete` + `git branch -D` steps; (c) replace BACKLOG.md update with no-op (lifecycle workflow handles `status:done`); (d) add `gh pr create` (if no PR exists) or `gh pr comment` (if one does) for UAT result; (e) add `Closes #N` reminder; (f) update frontmatter `name: speckit.uat.run`.
- [ ] T035 [US2] Write `/home/dominik/RedmineCalendar/.specify/extensions/uat/README.md` per `contracts/uat-extension-manifest.md` § README.md shape.
- [ ] T036 [US2] Delete `/home/dominik/RedmineCalendar/.claude/commands/speckit.uat.md` (the bespoke skill is now relocated to the extension).
...
- [ ] T038 [P] [US2] Update `/home/dominik/RedmineCalendar/CLAUDE.md`: in the Commands section, change `/speckit.uat` references to `/speckit.uat.run`. Note in the Project Structure section that `.specify/extensions/uat/` exists.
- [ ] T039 [P] [US2] Update `/home/dominik/RedmineCalendar/CONTRIBUTING.md`: same scope — slash command rename + extension-based architecture note.
...
- [ ] T040 [P] [US3] Diff `/home/dominik/RedmineCalendar/.specify/` against the 0.8.7-vanilla scratch from T003. Produce an inventory of project-specific files (anything not in vanilla, anything modified vs vanilla). Capture path, current_state, vanilla_baseline per `data-model.md` § Customization. Append the rows to the inventory table in `/home/dominik/RedmineCalendar/.specify/features/032-speckit-workflow-audit/research.md` § "Local customization inventory" (replacing the placeholder rows there with the real measured inventory).
- [ ] T041 [P] [US3] Same diff process for `/home/dominik/RedmineCalendar/.claude/`. Append inventory rows.
- [ ] T042 [P] [US3] Same diff process for `/home/dominik/RedmineCalendar/.github/` (note: GitHub Actions workflows + dependabot.yml + Issue templates are NOT vanilla Spec Kit — they're project-specific tooling, all in scope for the audit).
...
- [ ] T045 [US3] For the `PreToolUse` hook in `/home/dominik/RedmineCalendar/.claude/settings.json`, record the decision per FR-007. Recommended: **drop** (server-side branch protection now covers it; the local UX value is small relative to the maintenance surface). Apply the decision in the file: if `drop`, remove the entire `PreToolUse` block.
...
- [ ] T052 [P] Update `/home/dominik/RedmineCalendar/CHANGELOG.md` `[Unreleased]` section: add bullets summarising the workflow audit (Spec Kit version bump, BACKLOG → Issues migration, UAT extension, hook trim).
- [ ] T053 [P] Update `/home/dominik/RedmineCalendar/README.md` if any user-facing docs reference BACKLOG.md or the old slash command names.
...
- [ ] T058 Review the PR description on PR #31. Ensure it lists every behavior change for the team (BACKLOG → Issues, slash command renames, dropped hooks, version bump). Move PR #31 from draft to ready-for-review.
...
Phase 6 (Polish):      {T052, T053} (parallel) ─→ T054 ─→ T055 ─→ T056 ─→ T057 ─→ T058
```
...
All 58 tasks above follow the required format: `- [ ] T### [P?] [Story?] Description with file path`. Story labels appear on Phase 3-5 tasks only (matching the user-story phases); Setup, Foundational, and Polish phases carry no story label. Every task names its target file path (or directory).
