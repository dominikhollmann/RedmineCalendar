...
- Encrypted localStorage (AES-GCM via Web Crypto API) is the mechanism for per-user credential data, consistent with the implementation delivered by this feature. Each employee has a separate Windows login and therefore a separate browser profile — no multi-user-per-browser support is needed.
