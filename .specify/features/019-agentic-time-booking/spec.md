...
- Q: How should the browser app access the Microsoft Graph API? → A: Directly from the browser using MSAL.js. The company SSO (Azure AD/Entra ID) enables silent token acquisition — users are already authenticated via the same SSO session used for Outlook/Teams, so no popup or redirect is needed in the normal case.
- Q: How should the OAuth2 flow work for the user? → A: MSAL.js `acquireTokenSilent()` using the existing SSO session (Azure AD/Entra ID). No popup needed when SSO session is active. Popup fallback only if silent auth fails (first-time consent or expired session). Admin registers Azure AD app with `Calendars.Read` delegated permission and configures client ID in `config.json`. Each user's token grants access only to their own calendar (delegated permissions, no impersonation possible).
...
- Q: Where should OAuth2 refresh tokens be stored? → A: Managed by MSAL.js in its built-in browser cache (sessionStorage or localStorage, configurable). MSAL.js handles the full token lifecycle including silent refresh via SSO session.
...
- **Architecture**: The app is a static SPA served from a shared web server (established in feature 008). Phase 1 accesses the Microsoft Graph API directly from the browser using MSAL.js (Microsoft Authentication Library for JavaScript). Authentication leverages the company's existing Azure AD/Entra ID SSO — users are already logged in via the same session used for Outlook and Teams. No server-side proxy, local server, or additional login needed for calendar access. Later phases requiring local-only data sources (git, Windows Event Log) will need a local companion service — this is deferred to those phases.
- **Microsoft 365 access**: Outlook calendar data is accessed via the Microsoft Graph API. MSAL.js acquires tokens silently using the user's existing SSO session (`acquireTokenSilent()`). Each token uses delegated permissions (`Calendars.Read`) — the user can only access their own calendar, preventing impersonation. The admin registers an Azure AD app with delegated (not application) permissions and configures the client ID in `config.json`. Popup fallback is used only if the silent flow fails (first-time consent or expired SSO session). Teams, SharePoint, and email (later phases) use the same MSAL.js flow with additional scopes.
