...
- [x] T011 [TDD] Write unit tests for js/outlook.js in tests/unit/outlook.test.js BEFORE implementation (T005–T008): ticket extraction (single, multiple, none, edge cases), quarter-hour rounding, overlap detection, all-day event categorization, private event filtering. Tests MUST fail initially (Red), then T005–T008 make them pass (Green).
...
- [x] T012 [US1] Add `book_outlook_day` tool schema to `TOOL_SCHEMAS_CLAUDE` in js/chatbot-tools.js (auto-converted to OpenAI format via `toOpenAITools()` for ChatGPT/Codex compatibility per FR-008) — input: `date` (string, YYYY-MM-DD, defaults to today); description instructs LLM to present summary then walk through one-by-one
...
- [x] T016 [US1] Update system prompt in js/knowledge.js — add instructions for the booking flow: present summary first, then use `create_time_entry` for each confirmed meeting; handle multi-ticket meetings (ask user to pick one or split); add "outlook" and "book my time" to TOPIC_MAP keywords
...
- [x] T018 [US1, TDD] Write/update tests/unit/chatbot-tools.test.js BEFORE T012–T017: add mock for outlook module, test tool schema includes `book_outlook_day` when configured and excludes when not, test `executeBookOutlookDay` returns formatted summary. Tests MUST fail initially.
- [x] T019 [US1, TDD] Write UI test in tests/ui/outlook-booking.spec.js BEFORE T012–T017: test the booking flow: mock Graph API response, verify summary appears in chat, verify create_time_entry modal opens with correct prefill. Tests MUST fail initially.
...
Within Phase 2 (TDD):
  T011 first (write failing tests for outlook.js)
  T005 → T006 → T007 → T008 (implement to make T011 tests pass)
  T009, T010 (parallel with T005-T008: settings are independent)

Within Phase 3 (TDD):
  T018, T019 first (write failing tests for tool + UI flow)
  T012 → T013 → T014, T015 (implement to make T018/T019 pass)
  T016 (parallel with T012-T015: system prompt is independent)
  T017 (after T012+T013: wires schema + handler together)
```
