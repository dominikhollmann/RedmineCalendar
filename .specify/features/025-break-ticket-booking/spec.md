...
- **Calendar Event Routing Decision**: For each calendar event in the booking flow, the proposal carries one of three routings: (1) work ticket (extracted from title or asked), (2) holiday ticket (all-day OOO/holiday event), (3) break ticket (0 hours, AI-classified non-work or user-routed). This is a transient runtime concept, not stored. Outlook `sensitivity` and `showAs` flags do NOT influence the routing decision.
