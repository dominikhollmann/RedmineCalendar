...
3. Group remaining entries by `spentOn` (date string). Within each group, run `overlappingEntries(group)`: a pairwise pass that, for each colliding pair, records the reason string referencing the *other* entry on both sides of the pair.
