...
2. **Script fails** (not on a feature branch): You MUST prompt for available features (Scan `.specify/features/NNN-*/` to get available features). Use the **AskUserQuestion tool** to let the user select. **Do NOT guess or auto-select a change. Always let the user choose.**
