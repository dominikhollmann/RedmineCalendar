...
5. **Save report**: Write the bug report to `.specify/features/{feature}/bugs/BUG-{NNN}.md` where `{NNN}` is the next sequential bug number. Create the `bugs/` directory if it does not exist.
