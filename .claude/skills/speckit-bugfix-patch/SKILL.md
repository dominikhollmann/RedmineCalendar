...
3. Check for bug reports in `.specify/features/{feature}/bugs/` — if a bug ID is provided, load that report
...
1. **Load bug context**: Read the relevant bug report and all spec artifacts:
   - **Bug report**: `.specify/features/{feature}/bugs/BUG-{NNN}.md` (if specified)
