...
3. **Look up the Issue.**
   ...
   If empty, log `No Issue found for Feature ${NUM} — skipping (project may have drifted; run .specify/scripts/migrate-backlog-to-issues.mjs)` and exit 0. **This is graceful degradation, not an error** — the hook should never block a Spec Kit step.
